...
import { fetchAllStores, fetchAppFromApiKey, fetchOrgAndApps, fetchOrganizations, fetchOrgFromId, } from './dev/fetch.js';
...
import { updateAppIdentifiers, getAppIdentifiers } from '../models/app/app.js';
import { error as kitError, output, session, store, ui, environment } from '@shopify/cli-kit';
export const InvalidApiKeyError = (apiKey) => {
    return new kitError.Abort(output.content `Invalid API key: ${apiKey}`, output.content `You can find the API key in the app settings in the Partner Dashboard.`);
};
export const DeployAppNotFound = (apiKey, dependencyManager) => {
    return new kitError.Abort(output.content `Couldn't find the app with API key ${apiKey}`, output.content `• If you didn't intend to select this app, run ${output.content `${output.token.packagejsonScript(dependencyManager, 'deploy', '--reset')}`.value}`);
};
 ...
 * @returns
 */
function getAppDevCachedInfo({ reset, directory, apiKey, }) {
    if (!apiKey)
        return undefined;
    if (apiKey && reset)
